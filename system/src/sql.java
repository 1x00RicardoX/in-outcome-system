...
public class sql {
    ...
    public static String[] researInAll(int i) {//查询信息
    ...
    public static int alter(int i,String[] tmp) {//修改个人信息
        final int id=i;
    	ResultSet rs = null;
        Connection conn;
        Statement sm;
        String url = "jdbc:sqlserver://localhost:1433;DatabaseName=jdbctest1;";
        String[] sql= {"update userFile1 set account='",
        		"update userFile1 set password='",
        		"update userFile1 set name='",
        		"update userFile1 set sex='",
        		"update userFile1 set phone=",
        		"update userFile1 set mail='",
        		"update userFile1 set address='",
        		"' where id='"+id+"'\n",
        		"  where id='"+id+"'\n"};        				
        ...
        try {
            ...
            for(int i1=0;i1<7;i1++) {
            	if(i1!=4)
            		System.out.println("use jdbctest1;\n"+sql[i1]+tmp[i1]+sql[7]);
            	else
            		System.out.println("use jdbctest1;\n"+sql[i1]+tmp[i1]+sql[8]);
            }
            ...
            for(int i1=0;i1<7;i1++) {
            	if(i1!=4)
            		sm.executeUpdate("use jdbctest1;\n"+sql[i1]+tmp[i1]+sql[7]);
            	else
            		sm.executeUpdate("use jdbctest1;\n"+sql[i1]+tmp[i1]+sql[8]);
            }
